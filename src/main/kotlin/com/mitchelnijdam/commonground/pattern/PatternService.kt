...
class PatternService(
    ...
    fun activePatternsForTopic(topicId: Long): List<Pattern> {
        requireTopicExists(topicId)
        return patternRepository.findByTopicIdAndActiveTrue(topicId).sortedBy { it.title.lowercase() }
    }
    ...
    @Transactional
    fun create(topicId: Long, title: String, code: String, language: String): Pattern {
        ...
        return patternRepository.save(Pattern(topic = topic, title = title, code = code, language = language))
    }

    @Transactional
    fun deactivate(patternId: Long): Pattern {
        val pattern = patternRepository.findById(patternId)
            .orElseThrow { ResponseStatusException(HttpStatus.NOT_FOUND, "Pattern $patternId not found") }
